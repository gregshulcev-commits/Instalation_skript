...
После создания нового интерфейса скрипт предложит пересобрать nftables/NAT для всех интерфейсов. Это важно: firewall-файл должен содержать все ListenPort и все AWG interface names одновременно.
...
Основные файлы:
  ...
  /etc/amnezia/amneziawg/install.env
      Пути к awg, awg-quick, кешу исходников и sysctl-файлу.

  /etc/amnezia/amneziawg/firewall.env
      Внешний интерфейс и SSH-порт, использованные при последней генерации nftables.

  /etc/amnezia/amneziawg/manager.env
      Указатель на последний созданный/настроенный интерфейс. Оставлен для совместимости.
...
nftables
--------
Скрипт scripts/03_setup_nftables.sh собирает ruleset для всех найденных AWG конфигов:
...
Важное предупреждение: генерируемый nftables.conf содержит flush ruleset и заменяет текущие nftables-правила сервера. Если на сервере уже есть сложный firewall, сначала сохраните его и вручную объедините правила.
...
- nftables для одного интерфейса;
- добавление клиента IPv4-only;
- создание awg1 без поломки awg0;
- nftables для двух интерфейсов;
