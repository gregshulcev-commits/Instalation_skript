...
! grep -q 'restart awg-quick@awg1.service' "${TEST_LOG_DIR}/systemctl.log" || fail "Добавление клиента не должно делать полный restart awg-quick по умолчанию"
grep -q 'restart wgexporter' "${TEST_LOG_DIR}/systemctl.log" || fail "После добавления клиента мониторинг должен перезапускаться best-effort"
...
pass "04_add_client.sh добавляет клиента в выбранный интерфейс без полного restart awg-quick и обновляет мониторинг"
...
! grep -q 'restart awg-quick@awg1.service' "${TEST_LOG_DIR}/systemctl.log" || fail "Удаление клиента не должно делать полный restart awg-quick по умолчанию"
grep -q 'restart wgexporter' "${TEST_LOG_DIR}/systemctl.log" || fail "После удаления клиента мониторинг должен перезапускаться best-effort"
...
pass "08_remove_client.sh удаляет клиента без полного restart awg-quick, обновляет мониторинг, а restore возвращает backup"
